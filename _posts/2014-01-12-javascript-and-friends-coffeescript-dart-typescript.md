---
layout: default
title: "JavaScript and Friends: CoffeeScript, Dart, TypeScript"
date: 2014-01-12
---

# Contents

[Why JavaScript Isn't Enough?](#javascript)
[Example JavaScript Program: Dijkstra's Algorithm](#javascriptexample)
...
As an example, let's take an implementation of [Dijkstra's algorithm](http://en.wikipedia.org/wiki/Dijkstra's_algorithm) for finding the shortest path between two nodes in a directed graph. For the purposes of the present article it is actually not important to understand how the algorithm works, but in case you are interested in the algorithm itself, you can just read the Wiki [article](http://en.wikipedia.org/wiki/Dijkstra's_algorithm).

So, [here](https://github.com/antivanov/misc/blob/master/JavaScript/dijkstra_algorithm.js) is the implementation in JavaScript:
...
[CoffeeScript](http://coffeescript.org/) addresses some of the JavaScript issues described above. It introduces classes, shortcuts for most common JavaScript boiler-plate code, such as **@** for **this** and **::** for **prototype**, saves on number of code lines, gets rid of curly braces and actively uses indentation to give structure to your program.

[Here](https://github.com/antivanov/misc/blob/master/CoffeeScript/dijkstra_algorithm.coffee) is the algorithm rewritten in CoffeeScript:
...
As it can be seen CoffeeScript is a completely different language that can be then (usually) compiled into JavaScript or executed outright. It is not a superset or subset of JavaScript and you cannot just freely mix JavaScript and CoffeeScript constructs in one program. There is still a [mechanism](http://coffeescript.org/#embedded) for executing JavaScript embedded inside your CoffeeScript, although normally you would not use it.
...
[Here](https://github.com/antivanov/misc/blob/master/TypeScript/dijkstra_algorithm.ts) is the TypeScript version:
...
A bit about tools and editors. There is a special Dart [editor](http://www.dartlang.org/downloads.html) which has code assistance and highlighting and makes development much more pleasant. In the future there can also appear plugins for popular IDEs like, Eclipse.
...
![](http://smthngsmwhr.files.wordpress.com/2013/02/js_langs.png)
...
[Goto 2012: TypeScript Keynote](http://www.youtube.com/watch?v=3dqZW_DqHIQ)
[Dart language](http://www.dartlang.org/)
[Google I/O 2012: Dart - A Modern Web Language](http://www.youtube.com/watch?v=bsGgfUreyZw)
